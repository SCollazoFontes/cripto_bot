...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Optimización de la estrategia Momentum.")
    parser.add_argument(
        "--dataset",
        default="data/datasets/BTCUSDT_master.csv",
        help="CSV maestro de trades (salida de update_master_dataset).",
    )
    parser.add_argument("--symbol", default="BTCUSDT", help="Símbolo de trading.")
    parser.add_argument(
        "--windows",
        nargs="+",
        default=["30m", "1h", "3h", "6h", "12h", "1d", "3d", "7d"],
        help="Ventanas relativas/absolutas para evaluar (ej: 12h 1d 3d).",
    )
    parser.add_argument(
        "--optimizer",
    ...
    parser.add_argument(
        "--max-trials", type=int, default=20, help="Número máximo de combinaciones por ventana."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out-root",
        default="runs_opt",
        help="Directorio raíz donde se guardarán los resultados de optimización.",
    )
    parser.add_argument(
        "--min-improvement",
        type=float,
        default=0.0,
        help="Mejora mínima requerida para resetear la paciencia (score).",
    )
    parser.add_argument(
        "--patience",
        type=int,
        default=0,
        help="Número de trials consecutivos sin mejora significativa antes de detener la ventana.",
    )
    parser.add_argument(
        "--min-trades",
        type=int,
        default=5,
        help="Número mínimo de trades para considerar válido un resultado (penaliza runs con menos operaciones).",
    )
    parser.add_argument(
        "--builders",
        nargs="+",
        default=["hybrid_100ticks_all", "hybrid_100ticks_any", "compact_60ticks"],
