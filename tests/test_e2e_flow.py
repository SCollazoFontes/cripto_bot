...
async def test_websocket_connection():
    """Test 1: Verificar conexión WebSocket a Binance testnet."""
    logger.info("=" * 60)
...
async def test_full_flow_simulation():
    """Test 5: Flujo completo con datos reales (1 minuto)."""
    logger.info("=" * 60)
    ...
    try:
        ...
        builder = VolumeQtyBarBuilder(qty_limit=0.5)  # Barras pequeñas para testing
        ...
        logger.info(f"Capturando trades por {max_duration}s (max {max_trades} trades)...")
        ...
        async for trade_data in iter_trades("BTCUSDT", testnet=True):
            ...
            # Procesar con builder
            bar = builder.update(trade)
            if bar:
                bars_created += 1
                logger.info(
                    f"  📊 Bar #{bars_created}: OHLC=[{bar.open:.2f}, {bar.high:.2f}, "
                    f"{bar.low:.2f}, {bar.close:.2f}] vol={bar.volume:.6f} "
                    f"trades={bar.trade_count}"
                )

            # Log progreso cada 20 trades
        ...
        logger.info("=" * 60)
        logger.info(
            f"✅ Flujo completo exitoso: {trades_processed} trades → {bars_created} bars "
            f"en {elapsed:.1f}s"
        )
...
async def main():
    ...
    results = {
        ...
        "Full Flow (1min)": await test_full_flow_simulation(),
    }
